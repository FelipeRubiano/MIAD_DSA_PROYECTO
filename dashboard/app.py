...
app.layout = html.Div(
    [
        html.H2(
            "Modelo de Riesgo de abandono de Clientes",
        ...
        html.Div(
            [
                ...
                html.Div(
                    id="thr_lbl",
                    style={"alignSelf": "flex-end", "marginLeft": "12px"},
                ),
            ],
            style={
                "display": "flex",
        ...
        html.Div(
            [
                html.Div(
                    [html.Label("Filtro país"), dcc.Dropdown(id="f_country", placeholder="Todos")]
                ),
                html.Div(
                    [html.Label("Filtro género"), dcc.Dropdown(id="f_gender", placeholder="Todos")]
                ),
                html.Div(
                    [
                        html.Label("Filtro edad"),
                        dcc.RangeSlider(
                            id="f_age",
                            min=0,
                            max=100,
...
@app.callback(
    ...
    Output("g_imp", "figure"),
    Output("tbl_top", "data"),
    Output("tbl_top", "columns"),
    Output("f_country", "options"),
    Output("f_gender", "options"),
    Output("f_age", "min"),
    Output("f_age", "max"),
    Output("f_age", "value"),
    Output("f_income", "min"),
    Output("f_income", "max"),
    Output("f_income", "value"),
    Output("kpis", "children"),
    Input("file", "contents"),
    Input("thr", "value"),
    Input("f_country", "value"),
...
def run(contents, thr, v_country, v_gender, v_age, v_income, filename):
    ...
    opts_country = (
        [{"label": x, "value": x} for x in sorted(df_raw["country"].dropna().unique())]
        if has_country
        else []
    )
    opts_gender = (
        [{"label": x, "value": x} for x in sorted(df_raw["gender"].dropna().unique())]
        if has_gender
        else []
    )

    age_min, age_max = (
        (int(df_raw["age"].min()), int(df_raw["age"].max())) if has_age else (0, 100)
    )
    inc_min, inc_max = (
        (
            float(df_raw["estimated_salary"].min()),
            float(df_raw["estimated_salary"].max()),
        )
        if has_income
        else (0.0, 300000.0)
    )
    ...
    if has_country and v_country:
        mask &= df_raw["country"] == v_country
    if has_gender and v_gender:
        mask &= df_raw["gender"] == v_gender
    if has_age:
        mask &= df_raw["age"].between(age_val[0], age_val[1])
    if has_income:
        mask &= df_raw["estimated_salary"].between(inc_val[0], inc_val[1])
    ...
    df_pred["accion"] = np.where(
        df_pred["probabilidad"] >= thr,
        "Prioridad ALTA de retención",
        "Seguimiento normal",
    )
    ...
    kpis.append(
        kpi_box(
            "% Riesgo ALTO",
            f"{(df_pred['probabilidad'] >= thr).mean() * 100:.1f}%",
        )
    )
    ...
    if target:
        ...
        if y_true.dtype == "O":
            ...
            y_true = (
                pd.to_numeric(y_true, errors="coerce").fillna(0).astype(int)
            )
        ...
        kpis.extend(
            [
                ...
                kpi_box(
                    "KPI Accuracy",
                    f"{accuracy_score(y_true, df_pred['prediccion']) * 100:.1f}%",
                ),
                kpi_box(
                    "KPI F1-score",
                    f"{f1_score(y_true, df_pred['prediccion']):.2f}",
                ),
            ]
        )
    else:
    ...
    fig_dist = px.histogram(
        df_pred, x="probabilidad", nbins=20, title="Distribución de Riesgo"
    )

    if has_country and len(df_raw_f):
        tmp = pd.DataFrame(
            {"country": df_raw_f["country"], "prob": df_pred["probabilidad"]}
        )
    ...
    df_top = pd.DataFrame(
        {
            "probabilidad": df_pred["probabilidad"],
            "prediccion": df_pred["prediccion"],
        }
    )

    for cand in ["CustomerId", "customer_id", "CustomerID", "id", "ID"]:
